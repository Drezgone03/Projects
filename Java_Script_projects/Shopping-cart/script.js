...
function addToCart(element) {
  ...
  if (parseInt(quantity) > 0) {
    ...
    let zbir = document.querySelector(".total");
    zbir.innerHTML = `Total: $${totalEnd}`;

    element.innerText = "Dodato";
    element.setAttribute("disabled", "true");
  } else {
    alert("Unesite broj veci od 0");
  }
}
...
function removeFromCart(element) {
  ...
  let razlika = document.querySelector(".total");
  razlika.innerText = `Total: $${totalEnd}`;
